...
describe("ArkaMaster", function () {
  ...
  describe("ArkaMaster tests", function () {
    it("sets correctly arkaToken and oracle addresses, test storage", async function () {
      const { arkaERC20, arkaMaster, oracle } = await loadFixture(
        deployArkaContracts
      );

      expect(await arkaMaster.arkaToken()).to.equal(arkaERC20.address);
      expect(await arkaMaster.priceFeedEthUsd()).to.equal(oracle.address);
    });
  });
});
